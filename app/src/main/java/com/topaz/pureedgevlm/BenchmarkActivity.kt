...
class BenchmarkActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tvInfo = TextView(this).apply {
            text = "点「跑 Benchmark」：对 YOLO/场景/OCR/大模型 四个模型按线程 1/2/4/8 各跑若干次测速，结果存成 CSV。"
        }
        ...
        Thread {
            ...
            try {
                ...
                safeUi {
                    ...
                    tvInfo.text = "Benchmark 完成。"
                    setBusy(false)
                }
            } catch (e: Exception) {
                Log.e("MainActivity", "Benchmark 出错", e)
                safeUi {
                    tvInfo.text = "⚠️ Benchmark 出错：${e.message}（详见 Logcat tag=MainActivity）"
                    setBusy(false)
                }
            } finally {
                bmp?.recycle()
